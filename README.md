...
の順で改行または空白区切りで入力。 $^i\mathrm{P} _ {i+1}$ と $\hat{\mathrm{s} _ i}$ についての定義とモード番号についてはDescriptionを参照のこと。 $\hat{\mathrm{s}}$ の方向は $x$ 軸方向まわりの回転なら $"x"$ 、 $y$ 軸方向まわりの回転なら $"y"$ 、 $z$ 軸まわりの回転なら $"z"$ を入力。モード番号はコンマなしで入力。
